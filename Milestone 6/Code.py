...
def check(poi,poi2,poi3,object):
    if(poi.edges==object.edges):
        ...
        if(poi.edgelength==object.edgelength):
            object.paste()
        elif(poi2.edgelength==object.edgelength):
            object.paste()
        elif(poi3.edgelength==object.edgelength):
            object.paste()
            # object.edgelength.reverse()
            # temp=object.edgelength.index(max(object.edgelength))
            # object.edgelength=object.edgelength[temp:]+object.edgelength[:temp]
            # if(poi.edgelength==object.edgelength):
            #     object.paste()
            #     print(object.edgelength)
            # if(poi.edges%2==1):
            #     object.paste()
            # else:
            #     dotproduct=[]
...
poi2=copy.copy(poi)
poi3=copy.copy(poi)
poi2.edgelength=[0.25*i for i in poi.edgelength]
poi3.edgelength=[2.25*i for i in poi.edgelength]

print(poi.edgelength,poi2.edgelength,poi3.edgelength)

while(True):
    object=polygon(poi.edges)
    check(poi,poi2,poi3,object)
    x=file1.readline()
    if(x!='boundary\n'):
        break
